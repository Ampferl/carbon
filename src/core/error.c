...
ErrorInfor __carb_errorInfor;

static void error_printError();

void error_throwError(ErrorType type,Token *token){

    if(!__carb_errorInfor.type){
        __carb_errorInfor.type=type;
        __carb_errorInfor.row=token->row;
        __carb_errorInfor.column=token->column;
...
void error_printError(){
    ...
    type=__carb_errorInfor.type;
    row=__carb_errorInfor.row+1;
    column=__carb_errorInfor.column+1;
    if(type){
