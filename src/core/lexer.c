    ...
    while(cur<__lex_strLen){
        if(str[cur]=='\x0a'||str[cur]=='\x09'||
           str[cur]=='\x0d'||str[cur]=='\x20'){
            ...
            else if(str[cur]=='\x09'||str[cur]=='\x20'){
                __lex_curCol++;
            }
...
static int __convertUTF8(char *str,int index,char *str1,int index1){
    ...
    if(value>=0x0000&&value<=0x007f){
        ...
        str1[index1]=0xc0|((value>>6)&0xff);
        str1[index1+1]=0x80|(value&0x3f);
        len+=2;
    }
    else if(value>=0x0800&&value<=0xffff){
        str1[index1]=0xe0|((value>>12)&0xff);
        str1[index1+1]=0x80|((value>>6)&0x3f);
        str1[index1+2]=0x80|(value&0x3f);
        len+=3;
    }
